...
# the vehicle loop can abort after this many iterations, when given a
# positive integer.
MAX_LOOPS = None

# =================================CAMERA======================================
# (PICAM|WEBCAM|CVCAM|CSIC|V4L|MOCK)
CAMERA_TYPE = "WEBCAM"
IMAGE_W = 160
IMAGE_H = 120

# default RGB=3, make 1 for mono
IMAGE_DEPTH = 3
CAMERA_FRAMERATE = DRIVE_LOOP_HZ

# For CSIC camera - If the camera is mounted in a rotated position, changing
# the below parameter will correct the output frame orientation
# (0 => none , 4 => Flip horizontally, 6 => Flip vertically)
CSIC_CAM_GSTREAMER_FLIP_PARM = 0

# 9865, over rides only if needed, ie. TX2..
# I2C address, use i2cdetect to validate this number
PCA9685_I2C_ADDR = 0x40

# None will auto detect, which is fine on the pi. But other platforms should
# specify the bus num.
PCA9685_I2C_BUSNUM = None

# ==============================DRIVETRAIN=====================================
# These options specify which chasis and motor setup you are using. Most are
# using SERVO_ESC. DC_STEER_THROTTLE uses HBridge pwm to control one
# steering dc motor, and one drive wheel motor DC_TWO_WHEEL uses HBridge pwm
# to control two drive motors, one on the left, and one on the right.
# SERVO_HBRIDGE_PWM use ServoBlaster to output pwm control from the PiZero
# directly to control steering, and HBridge for a drive motor.

# SERVO_ESC|DC_STEER_THROTTLE|DC_TWO_WHEEL|SERVO_HBRIDGE_PWM
DRIVE_TRAIN_TYPE = "SERVO_ESC"

# =============================STEERING========================================
# channel on the 9685 pwm board 0-15
STEERING_CHANNEL = 1
...
THROTTLE_FORWARD_PWM = 700  # pwm value for max forward throttle
THROTTLE_STOPPED_PWM = 370  # pwm value for no movement
THROTTLE_REVERSE_PWM = 220  # pwm value for max reverse throttle

# ==========================DC_STEER_THROTTLE==================================
# with one motor as steering, one as drive
...
# ============================DC_TWO_WHEEL=====================================
# with two wheels as drive, left and right.
...
# override this setting by passing the command
# line parameter --type to the python manage.py train and drive commands.

# (linear|categorical|rnn|imu|behavior|3d|localizer|latent)
DEFAULT_MODEL_TYPE = 'linear'

# how many records to use when doing one pass of gradient decent. Use a
# smaller number if your gpu is running out of memory.
BATCH_SIZE = 128

# what percent of records to use for training. the remaining used for
# validation.
...
# keep images in memory. will speed successive epochs, but crater if not
# enough mem.
CACHE_IMAGES = True

# This will remove weights from your model. The primary goal is to increase
# performance.
PRUNE_CNN = False

# The desired percentage of pruning.
PRUNE_PERCENT_TARGET = 75

# Percent of pruning that is perform per iteration.
PRUNE_PERCENT_PER_ITERATION = 20

# The max amount of validation loss that is permitted during pruning.
...
# be used to copy the final model to your vehicle. If not using this option,
# no need to set these.

# username on pi
PI_USERNAME = "pi"

# password is optional. Only used from Windows
# machine. Ubuntu and mac users should copy their public keys to the pi.
...
# the location of the mycar dir on the pi. this will be used to help locate
# the final model destination.
PI_DONKEY_ROOT = "/home/pi/mycar"

# ===============================CROP IMAGE====================================
# Region of interest cropping only supported in Categorical and Linear
# models. If these crops values are too large, they will cause the stride
# values to become negative and the model with not be valid.
...
# When copying weights during a model transfer operation, should we freeze
# a certain number of layers to the incoming weights and not allow them to
# change during training?

# default False will allow all layers to be modified by training
FREEZE_LAYERS = False

# when freezing layers, how many layers from the last should be allowed to
# train?
NUM_LAST_LAYERS_TO_TRAIN = 7

# ================================JOYSTICK=====================================
# when starting the manage.py, when True, will not require a --js option to
# use the joystick
...
# manually toggle recording with some other trigger. Usually circle button
# on joystick.
...
# SOMBRERO
# set to true when using the sombrero hat from the Donkeycar store.
# This will enable pwm on the hat.
HAVE_SOMBRERO = False
...
# COMMON ANODE? Some RGB LED use common anode. like
# https://www.amazon.com/Xia-Fly-Tri-Color-Emitting-Diffused/dp/B07MYJQP8B
LED_INVERT = False

# LED board pin number for pwm outputs These are physical pinouts. See:
# https://www.raspberrypi-spy.co.uk/2012/06/simple-guide-to-the-rpi-gpio
# -header-and-pins/
LED_PIN_R = 12
LED_PIN_G = 10
LED_PIN_B = 16
...
# ===============================BEHAVIORS=====================================
# When training the Behavioral Neural
# Network model, make a list of the behaviors, Set the TRAIN_BEHAVIORS =
# True, and use the BEHAVIOR_LED_COLORS to give each behavior a color
TRAIN_BEHAVIORS = False
BEHAVIOR_LIST = ["Left_Lane", "Right_Lane"]

# RGB tuples 0-100 per chanel
BEHAVIOR_LED_COLORS = [(0, 10, 0), (10, 0, 0)]

# =================================Localizer===================================
# The localizer is a neural network that can learn to predice it's location
# on the track. This is an experimental feature that needs more
# developement. But it can currently be used to predict the segement of the
# course, where the course is divided into NUM_LOCATIONS segments.
TRAIN_LOCALIZER = False
NUM_LOCATIONS = 10

# when enabled, makes it easier to divide our data into one tub per track
# length if we make a new tub on each X button press.
BUTTON_PRESS_NEW_TUB = False

# ===============================DonkeyGym
# ===================================== Only on Ubuntu linux, you can use
# the simulator as a virtual donkey and issue the same python manage.py
# drive command as usual, but have them control a virtual car. This enables
# that, and sets the path to the simualator and the environment. You will
# want to download the simulator binary from:
# https://github.com/tawnkramer/donkey_gym/releases/download/v18.9
# /DonkeySimLinux.zip then extract that and modify DONKEY_SIM_PATH.
DONKEY_GYM = False
...
# this keypress will enable this boost. It must be enabled before each use
# to prevent accidental trigger.
...
# 255, 255 is the center of the map. This offset controls where the origin
# is displayed.
PATH_OFFSET = (0, 0)
